...
- **weapons.js** — live: `currentWeapon, isAiming, switchAnim, meleeT`; `initWeapons()` (model+tangan, parent kamera, `scene.add(camera)`; rifle = Pindad SS2-V2 serba hitam sejak 2026-07-06: carry handle berkanal bidik ADS, handguard berusuk, popor rangka, mag STANAG Group, pose istirahat mag di konstanta `MAG_REST` — titik muzzle tetap z −7.4, mekanik tak berubah), `attachMuzzle(wpn)`, `startSwitch(t)`, `trySwitchKey(k)`, `startReload()`, `tryMelee()`, `toggleAim()`, `setAiming(v)`, `doMeleeHit()`, `updateWeaponTimers(dt)` (switch swap di tengah + melee hit 45%), `updateWeaponState(dt)` (recoil/heat decay + posisi z), `updateShooting()` (spread kerucut kamera + kick + spawn peluru; segmen frame-1 dari MATA), `updateWeaponVisuals(dt)` (ADS/FOV/bob/rig reload KF diskalakan reloadMs/melee/forearm), `resetWeapons()`, `placeLimb(...)`.
