...
| `shopKey(key)` → bool | opsional | core/input keydown | shop antar-gelombang survival: SPACE/Enter = Start Next Wave; angka 1-5 membeli SAAT overlay terbuka; return true = tombol DIKONSUMSI (tidak bocor ke ganti senjata / lompat) |
| `shopClose()` | opsional | core/input releaseInputs | tutup overlay shop saat unlock/blur/reset/game over (scene membuka lagi otomatis saat resume bila masih fase 'shopping') |
...
- **state.js** — status inti: `isPaused, isGameOver, score, mode, highScore, difficulty` (+ setter `setPaused/setGameOver/setScore/addScore/setMode/setHighScore/setDifficulty` — high score per difficulty: kunci `gibsHighScore_<diff>`, kunci lama fallback normal); `stats` (kills/headshots/shots/hits — per PELURU) + `resetStats()`; `player` (hp/ammo/mags/grenades/isReloading/lastShot/reloadTimer/reloadDurMs/speed/radius/vy/onGround; senjata: rifle/pistol/shotgun; `owned:{pistol,rifle,shotgun}` — Survival mulai pistol saja, mode lain semua; field dorman `dmgMul/reloadMul/upDmg/upReload` = 1/0) + `configurePlayer()` (stempel CFG; `owned` di-set per `mode`: survival = pistol saja); `keys, mouse`; array entitas `bullets, zombies, grenades, explosions, drops`; resource bersama `GEO, MAT`; vektor scratch `_dir,_right,_tip,_v3,_sRight,_sUp,_kickEuler`; `clearArray(arr, scene)`.
...
- **input.js** — `initInput()` (pointer lock + mouse look + klik + keyboard + blur + beforeunload; tombol senjata 1/2/3/Q; delegasi shop: tiap keydown ditawarkan dulu ke `activeScene.shopKey?.(key)` — true = dikonsumsi, digit tidak bocor jadi ganti senjata & SPACE tidak bocor jadi lompat [SPACE = Start Next Wave saat shop terbuka]), `requestLock()`, `enterImmersive()` (fullscreen + Keyboard Lock `LOCK_KEYS`, tanpa Escape), `releaseInputs()` (juga memanggil `activeScene.shopClose?.()`).
...
- **survival/index.js** — `survivalScene` (+hook `shopKey`/`shopClose`). **Wave berbasis "clear" (overhaul 2026-07-07)** via `wave.phase` (`'fighting'`→`'cleared'`→`'shopping'`): `startWave(n)` (jatah `zombiesPerWaveBase + (n-1)·Step`, cadence & cap dari `CFG.survival`, picu event bila `n % eventEveryWaves`), `updateMode` (spawn sampai jatah habis dibatasi `maxConcurrent`; bersih [`toSpawn===0 && zombies.length===0`] → 'cleared' + "WAVE CLEARED" + hitung mundur `shopCountdownSec` → `openShop()` + 'shopping'; fase shopping membuka ulang overlay bila sempat tertutup), `startNextWave()` (tutup shop + `startWave(num+1)`), `shopKey(key)` (SPACE/Enter → Next Wave, sisanya → `shopBuyKey`). `spawnZombie()` (lompat pagar dua fase + roll VARIAN per wave + `target:'monas'`), `monasHp` (objektif; 0 → `gameOver(false,'THE MONUMENT HAS FALLEN')`), `EVT` event wave (fog/blackout dari preset outdoor), `zombieAI` (jumping→chasing via `navAim`; penggerogot monas return `{}`). `hudStatus`: fighting `Wave N — X left · Monas P%`, cleared hitung mundur, shopping "Field Shop open".
- **survival/shop.js** — shop antar-gelombang (keyboard-only, pointer lock TETAP, game tidak pause; dibuka OTOMATIS oleh scene setelah wave selesai): `shopBuyKey(key)` (1=Replenish All Ammo [senjata DIMILIKI saja], 2=Replenish Grenades [penuh], 3=Replenish Health [penuh], 4=Buy Shotgun, 5=Buy Assault Rifle → `player.owned[w]=true` + loadout penuh; skor = mata uang via `addScore(-cost)`; ditolak bila penuh/dimiliki/skor kurang — skor utuh, `buyWeapon` kembalikan pesan), `openShop/closeShop/isShopOpen`, `render()` (item + baris `.shopNext` "START NEXT WAVE"). Tombol Next Wave (SPACE) DIMILIKI scene (`survival/index.shopKey`). Semua teks English.
...
| `shop` | ammoCost, grenadeCost, healthCost, shotgunCost, rifleCost (harga item shop antar-gelombang) |
