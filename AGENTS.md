...
**Stage 10 brutal siege-bomber + correct directional bank (2026-08-30, user request):** the final boss is `brutal-siege-bomber`, not the old rectangular wing/body. Its one prebuilt rig keeps the established local `+Z` nose and 132-unit span, but now has a layered armored keel/spine, cheek fangs, faceted canopy, twelve-piece cranked/razor wing planform, twin canted tail fins, four cowled turbofans with independently spinning rotors and pulsing exhausts, three twin-barrel armored turrets, two open weapon pods and exactly eight visible missiles. This is visual-only geometry: no gameplay HP/damage/radius changed, runtime allocates nothing, and the boss owns zero PointLights. Bank follows ACTUAL horizontal motion with `boss.bankDeg` and `boss.bankResponsePerSec`, including the leftward intro and damped direction reversals. The sign is load-bearing: with the airframe nose at local `+Z`, aerodynamic right is local `-X`; carrier yaw `PI` maps that anchor to world/screen right, so movement toward world `+X` requires POSITIVE Z roll to lower the right wing, while movement toward `-X` requires negative roll. Never restore the old `-boss.dir` sign. Debug/smoke pins the semantic wing anchors, both real travel directions, visual census, smooth configured limit and zero-light/prebuilt contract.

**Stage 11 Chapter 3 root-chamber overhaul (2026-08-30, user request):** the root chapter is a clean 100-metre approach to one open circular hall. `S11_ROOT_START` is exactly `100 * CAMP_M` east of the large hall door and `stage11RootMeterAt` advances toward world `-X`; `rootCorridor.meters` must remain 100 and `encounterMeter` 50 because `ensureStage11RootWorld` validates both against authored geometry after config load (never dereference `CFG` at module scope). The corridor has zero rails/walls, threshold columns or decontamination arches; the hall has zero buttresses, radial trusses or static capacitor-bank walls. Do not confuse those deleted static banks with the Warden's attached gameplay weak points, which remain. At exactly metre 50, two prebuilt shared `spawnMachine` rigs activate and print one bounded weighted-round-robin plan of exactly 12 C / 8 B / 4 A; no Chapter-3 robot exists before the line, counts are not difficulty-scaled, printed bots hold AI ownership until their birth transform finishes, and no obsolete `encounters.rootApproach` waves remain. At metre 100 a prebuilt monumental two-leaf door is a real collision/LOS blocker. Approaching `S11_DOOR_STAND` opens the actual shared Stage-1 `beginHackMinigame` ICE BREACH; abort/failure requires stepping away to re-arm, success alone slides both leaves apart over `doorOpenSec`, drops the blocker, hides the door marker and reveals the central-computer marker. Dynamic leaves must never enter `addMergedStaticShadowAware`. `S11_INSERT` equals the exact arena centre, and its solid pedestal's approach marker remains collision-reachable. The Warden starts 165 units on the far side so its orbit clears the computer. The computer is the root world's one legitimate occluder; deleted pillars/walls must not be restored merely to satisfy the world occlusion census. Smoke `25d` pins the geometry, empty first 49 m, two zero-light rigs, partial birth in progress, exact final class census, real modal hack, closed/open leaf poses, central interaction and continued Warden/upload flow.

**Stage 11 Chapters 2-3 share the lower-right camera (2026-08-30, user request):** the civic-axis and root/Warden chapters use the same immutable `STAGE11_CHAPTER_CAMERA`, derived from the shared camera's pitch/distance but placed at positive world X and positive world Z (southeast / screen lower-right). Both routes progress predominantly toward world `-X`; under this camera that projects to positive screen-up AND screen-left, so the player advances toward the upper-left. Chapter 2's reveal starts on the same side and must return to this offset after its cinematic rather than `null`/the global southwest camera. Chapter 3 owns it for the entire console approach and Warden battle. Both entries initially face the player toward world `-X`; mouse aim remains authoritative afterward. Occlusion follows the active scene offset automatically. Smoke `25d` pins corner signs, upper-left projection, exact offset continuity across the chapter handoff and persistence after Warden activation.

**Stage 11 hostile-pickup cinematic destruction (2026-08-30, user request):** weapon pickups no longer jump directly from alive to one generic `explodeAt` plus a tilted body. Every one of the six slots prebuilds its own zero-PointLight wreck FX rig: exactly `wreck.blastCount` staged flash/ring pairs, `fireCount` fires, `smokeCount` puffs and `sparkCount` sparks. The actual Stage-7 pickup parts are the debris—four tagged wheels, tagged hood, two bed sides, tailgate, plus the mounted weapon turret—so `roadVehicles.js` semantic `stage7WreckLoose` tags must remain and runtime must not create substitute meshes. Death immediately releases gameplay/gate ownership but enters `destroying` for `wreck.durationSec`: the body hops, paint/materials char, parts fly with deterministic ballistic velocities, three blasts fire at 0/`secondaryBlastSec`/`finalBlastSec`, persistent fire/smoke follows, and everything settles into a sunken tilted wreck. The combat camera holds its Pasupati framing through `wreck.cameraHoldSec` so the staged destruction is visible, then may ease back while the last smoke fades. Reset restores every original part transform and material exactly. All timing, motion, counts and shake values live under `campaign.stage11.forestVehicles.wreck`; smoke `25d` pins prebuilding, zero lights, all three stages, 9+ real breakaways, camera hold, full settlement and cleanup.

**Stage 10 boss faces and enters screen-up (2026-08-30, user report bomber looked like it flew backward):** the bomber mesh remains authored with its semantic nose at local `+Z`, but the carrier now has `rotation.y = PI`, mapping that nose to world `-Z`, which is screen-up under Stage 10's exact-top-down `cameraUp = (0,0,-1)`. Its intro path was reversed with the visual: it starts offscreen bottom-right and interpolates toward the upper hold position while world Z decreases, so do not restore the old top-to-bottom entry or rotate only the carrier while leaving a tail-first path. Starting laterally at `boss.sweepHalfWidth` keeps its 132-unit wing from crossing the player during the climb. Because the three turret yaws are local to this rotated carrier, runtime subtracts the carrier yaw from their world target angle; projectile muzzle anchors and aim therefore remain correct. Smoke `25c2` pins the PI yaw, bottom-right start, decreasing-Z forward entry, centring and continued turret fire.

**Stage 11 Chapter 2 LOOKS like 2045 now, and its plaza dressing follows one hard rule (2026-08-30, user: "perbaiki lagi desain tampilan world chapter 2 ... bayangkan sebuah kota modern canggih di tahun 2045"):** the skyline read as a HERITAGE complex, not a capital — `civic-palace` and `cultural-hall` were capped with tile-brown (`0x5b4d3b`) CONES, `civic-spire` was a cylinder + cone + stacked toruses (a pagoda), and only 3 of 10 archetypes used glass at all. Four changes. (1) **One shared facade language.** `glazingBands()` lays repeated horizontal curtain-wall bands, alternating lit/unlit on a deterministic hash so a repeated archetype never reads as the same building copied down the street; `briseSoleil()` adds vertical sun louvres (the detail that says built FOR this climate); `plantedLip()` plants every setback. Every cone roof and the whole brown roof material are deleted — smoke counts `coneRoofs === 0`, rejects the roof material by name, and requires every BUILDING to carry glazing and every VERTICAL archetype at least three bands (halls and stations are horizontal and correctly use one large glazed face). (2) **The transit viaduct became a MAGLEV** — slim guideway with a lit strip on tapered Y-piers, and a nose/four-body/tail set with a continuous window band, instead of five detached boxes floating over a concrete beam — plus three **air-taxi pads**, the single clearest read that this is not a present-day city. (3) **The plaza is inhabited**: lighting masts, a planted avenue, civic status totems and parked autonomous shuttles, with paving joints, edge light strips, water-garden kerbs and a shuttle-lane marking on the floor itself. (4) **THE RULE THAT MATTERS — anything with VOLUME stands OUTSIDE the walk bounds, anything inside them is FLAT.** A drawn prop the player walks straight through is exactly the "what is drawn is what blocks" violation the campaign forbids, and the fight's cover is authored separately in `addCivicCover()` and must never move for a visual pass. So all furniture sits on the terrace bank just past `PLAY.z` — close enough to touch from the high ground, impossible to walk into — and the plaza floor gets inlay only. Both halves are MEASURED from the transforms actually built (`plazaDressing.promenadeMinAbsZ` must exceed the play bound, `inlayMaxY` must stay under 2) rather than asserted in a comment. Cost: raw meshes 862 -> ~2000 for 2.3x the detail, welded draw groups essentially unchanged, blockers/nav/PointLights bit-identical. -> [docs/campaign.md](docs/campaign.md)

**Stage 11 Chapter 2 was FLAT and is now the last real level before the boss (2026-08-30, user: "stage 11 chapter 2 kok terasa terlalu flat ya, membosankan ... biar lebih menantang dan lebih memorable karena ini akan jadi stage terakhir sebelum stage melawan boss"):** it was measurably flat in three separate ways at once. `stage11SurfaceWalk` was four comparisons over a bare 1620x480 rectangle; `stage11SurfaceGroundHeight` returned a constant 0, so every terrace, water garden and colonnade the world draws sat OUTSIDE the walk box or could never be stood on; and the whole chapter was one mechanic run five times (clear batch, walk 240 units, repeat) with no second threat type at all. Worst of all, 97 robots batched at 24 gave `[24, 24, 24, 24, 1]` — **the last fight before the Warden was a single class C.** Four changes, each config-owned under `campaign.stage11.civicAxis`. (1) **THREE AUTHORITY PYLONS replace the straight lane.** They stand off the centre line on ALTERNATING sides and descending x, so the route is a zig-zag rather than a corridor; the descent iris stays sealed until all three are down, and killing every robot on the map opens nothing. Each pylon is defended by ONE whole configured `encounters.civicAxis` formation — the config already held exactly three waves, so the split is derived rather than re-authored, and it is what removes the one-robot finale. (2) **The WARDEN SUPPRESSION SWEEP makes the open ground lethal**, which is what turns the drawn plaza into a level. It is a constant-LINEAR-width wall at constant speed, so **dwell is `2 * halfWidth / speed`, identical everywhere, and must exceed `lockSec`** — published as `dwellExceedsLock` and smoke-asserted, because an earlier Stage 11 scan band died exactly here (1130 units in 3.5 s against a 28-unit half-width = 0.17 s inside a 1.2 s lock). **A rotating beam was rejected for the mirror-image reason**: its linear width grows with distance, so it is undodgeable at the far end of a 1600-unit axis. Safety is REAL GEOMETRY — a blocker within `coverShadowUnits` on the side the wall is coming FROM — so every column casts a readable shadow along the axis and the safe side flips when the wall turns. It hunts only the player; damaging its own garrison would make it a farming tool. (3) **The axis SEALS behind each fallen pylon**: `lockdownX` retreats to `pylon.x + trailUnits`, `stage11SurfaceWalk` follows it, and a drawn curtain stands on exactly that line (never an invisible wall), rising from below the opaque civic slab rather than being un-hidden. (4) **The flanking banks are real HIGH GROUND.** `stage11SurfaceGroundHeight` is now an analytic stepped field matching the drawn ceremonial stair tread-for-tread, the play box widened to reach it, and the near city band was pushed out so the widened area cannot touch the blocker-free welded clusters. Stepping up needs no jump because `updatePlayer` snaps feet to `groundHeight`; **robots need `syncRobotToTerrace` BEFORE `campaignRobotAI`** (the Stage 7 road pattern) because that helper pins `groundY = 0` on the idle->chasing transition, and `clampDropPos` returns a third element so loot on the bank stays on the bank. The plateau deliberately carries NO cover, so high ground trades sightlines for exposure to the sweep. -> [docs/campaign.md](docs/campaign.md)

**Stage 11 Chapter 1 is gated by FABRICATOR CHECKPOINTS, and shelled over its last third (2026-08-30, user request "di setiap 50 meter akan ada muncul 2 mesin spawn robot ... player harus menghancurkan dulu mesin itu baru bisa melanjutkan perjalanan", retuned the same day to "setiap 50 meter terlalu dekat"):** the checkpoint metres are the AUTHORED list `forestCheckpoints.meters` — **100, 175, 250, 350, 425, 500, 600, 675, 750** — because an even 50 m spacing ran one fight straight into the next. Each carries **2 shared spawn machines plus a garrison of exactly 10 C / 5 B / 5 A**, and a full-width hazard barrier `gateAheadMeters` (15 m) further on. **A gate opens on TWO conditions, not one**: every fabricator at that metre destroyed, AND — at the metres that also carry weapon vehicles (250 / 500 / 750, 2026-08-30 follow-up "pastikan juga player sudah menghancurkan mobil musuh sebelum bisa melanjutkan perjalanan") — every hostile pickup at that metre destroyed too. Killing the garrison robots is never enough for either half. **The metre table is DERIVED, never authored**: it is generated from the spacing, and the metres where the double-cabin weapon vehicles already stand (250 / 500 / 750) automatically take the lighter `vehicleCheckpoint` shape of **1 machine + 5 C / 3 B / 2 A**, so retuning `forestVehicles.checkpoints` moves the lighter checkpoints with it instead of leaving a second table to drift. Seven rules. (1) **The barrier is a real blocker on exactly the line it is drawn on** — no invisible wall — spanning at least the walk predicate's own half-width (`p.w`) so its ends cannot be a gap beside a solid-looking rail, and 11 units thick against a player who covers at most `speed * stepMax` (~4.5 units) in one frame, so it cannot be tunnelled. (2) **It SINKS below the opaque asphalt when cleared, never `visible=false`** (the Stage 3 deploy rule), so its materials have been drawn since the stage's first frame and opening a gate can never trigger a shader recompile; its status lamp material is **cloned per gate**, because one shared instance would turn every barrier in the forest green the moment a single checkpoint fell (the Stage 9 fuel-indicator bug). (3) **Clearing is POLLED in the frame update, never done inside `destroyMachine`** — at a vehicle metre the last blocker can just as easily be a weapon pickup, whose death happens inside `forestVehicles.js` with no hook of its own; `vehiclesLeft` asks that module directly and is keyed off the same derived `kind`, so a metre without vehicles can never ask about them. The radar and HUD follow the same split: once the machines are down the marker moves to the surviving vehicles. (4) **Only the NEXT uncleared checkpoint may arm.** The gates already make the sequence strictly ordered, but stating it in `updateStage11ForestCheckpoints` is what bounds the live population to one checkpoint even if the player is ever placed further along. (5) **A cleared checkpoint takes its garrison with it** (`collapseOnClear`, the Stage 7 factory-network contract): what is on screen dies through the normal explosion path with full gore and loot, what is off screen is removed silently rather than scattering pickups nobody walks back for. Without it every gate's survivors would follow the player into the next gate and fifteen checkpoints would grow the live population without bound. (6) **A fabricator never stands on the centre line and its clearance search only pushes it FURTHER out**, because the wreck stays solid after the fight — a placement that sealed the road would lock the player in forever; smoke measures a contiguous free lateral run at every checkpoint metre. The garrison spread is likewise capped short of its own barrier, or a robot would be born on the unreachable side of it. (7) **`bulletBlocked` calls the fabricator hit test BEFORE the wall sweep** (the Stage 6 HQ ordering rule), because the chassis is also a bullet-stopping blocker and testing the sweep first would delete the round as a wall hit without ever damaging the machine. HP stays the ONE shared `campaign.spawnMachine.hp`; everything else is `campaign.stage11.forestCheckpoints`. **A MORTAR BARRAGE covers metres 550-750** (`campaign.stage11.forestMortar`, same user message: "tambahkan tembakan mortar musuh seperti di stage 7"), reproducing Stage 7's artillery contract exactly: it may launch only while the player's ROUTE METRE is inside the window, one ballistic shell every `intervalSec` from behind the tree line ahead of the player on alternating sides, the shell corrects toward the player until the final `lockSec` and then FREEZES its marked impact point (so the tracking ring is a promise you can outrun, not a homing hit), and the queued blast deliberately ignores road-level cover between impact and the robots it catches because it falls from above. Its radius is read from `bosses.tank.mortarBlastRatio` rather than copied, the Stage 5 locomotive rule, so every mortar in the game stays one size. **Fixed along the way — an OBB collider built from `axz: +sin(yaw)` is MIRRORED**: `rotation.y = yaw` puts local +X at `(cos yaw, -sin yaw)`, so writing `+sin` rotates the collider by `-yaw` while the mesh turns by `+yaw`. On this route's straight first leg `sin ~ 0` hides it completely; on its bends it is up to 39 degrees out, and that is exactly what let the player SLIDE along a closed barrier's face and come out past it, because a mis-rotated face carries you forward as you slide. `forestWorld.js`'s `blocker()` now writes the drawn axes, the gate box takes its axes STRAIGHT from the route tangent rather than round-tripping through a yaw, and smoke pins that relation. **`stage11/rootWorld.js` and `surfaceWorld.js` still carry their own copies of the mirrored helper** — not touched here because their callers and geometry need their own verification. → [docs/campaign.md](docs/campaign.md)

**Stage 11 vehicle MG shared-bolt + camera arming delay (2026-08-30, user request):** Chapter-1 pickup machine guns use the same standard `makeEnemyBulletMesh` bolt silhouette as the Stage-4 tank and Stage-5 locomotive, not the elongated Combat Gunship tracer. They keep their configured 20-round/10-damage burst, target the player's live position per round like the tank, and remain pooled. `machineGun.bulletSpeed` is 4 baseline-frame units and its human-readable `bulletLifeSec` is converted to the shared enemy-bullet runtime's 60-Hz lifetime units at spawn. Every hostile pickup, including homing-missile cars, starts `forestVehicles.engagementDelaySec` (2 s) when its centre first enters the player camera: the checkpoint camera begins its Pasupati zoom immediately, the turret may visibly aim, but no projectile may launch before `fireReady`. Leaving the frame resets the countdown so a car cannot arm off-camera. Config/debug/smoke pin both the shared bolt dimensions and the full two-second no-fire window.

**Stage 11 initial-loading config safety (2026-08-30, user report loading stuck on `undefined.stage11`):** `CFG` is intentionally `{}` while campaign modules are imported and is only populated by `await loadConfig()` in `main.js`, so Stage 11 modules must never dereference `CFG.campaign.stage11` at module scope. The forest route's 800-metre length is structural geometry exported as `S11_FOREST_ROUTE_METERS = 800`; `ensureStage11ForestWorld` validates the matching JSON value only after loading has completed. Keep all tunable vehicle/camera/encounter reads inside functions. Smoke `25d` source-guards the route export so the boot-time dereference cannot return.

**Stage 11 forest enemy vehicles are gameplay-config owned (2026-08-30, user request):** every tunable value for the six hostile pickups lives under `campaign.stage11.forestVehicles` in `config/gameplay.json`, never in `forestVehicles.js`/`forest.js`/`forestWorld.js`. Common config owns asset key, activation range, aim/flash response, body colours, normal/combat camera offsets/ease, wreck sink/tilt/ease and each checkpoint's exact `{weapon,lateral}` vehicles. `machineGun.hp` and `homingMissile.hp` are the VEHICLE/GUNNER target durability and must overwrite the spawned B/A robot's `hp` AND `maxHp`; `homingMissile.projectileHp` is separately the shootable missile durability. Each weapon block also owns its robot class, damage, speed, lifetime, cadence/range/blast/hit values and pool size. `applyDifficulty` explicitly scales both vehicle HP values, missile projectile HP and both damage values because these no longer borrow difficulty-mutated robot class health. Debug/smoke must compare live rider/projectile/bullet values and camera/checkpoint layout directly against the config.

**Stage 11 Chapter-1 Stage-7 vehicle/camera/asphalt correction (2026-08-30, user report):** every Chapter-1 car now comes from `stage7/roadVehicles.js`: the sixteen merged roadblocks and all six hostile double-cabin weapon pickups use the Stage-7 `pickup` builder, with only the second cab row and weapon rig added afterward. Do not restore `enemyPickup.js` as their chassis. The green "asphalt" bug was a missing road, not a wrong colour: the indexed miter ribbon triangles were wound downward, so FrontSide culling exposed the green forest plane underneath. Road indices are counter-clockwise from above, normals face upward, asphalt remains `0x303638`, and smoke pins that it differs from the forest ground. The seven route legs still sum exactly 800 m but alternate through six deliberate turns (up to ~37 degrees), rather than reading as one straight line. Chapter-1 gameplay camera offset is `(70.7,116,-70.7)`, which projects forward route travel toward screen-right + screen-up. When the first living vehicle of a checkpoint becomes visible, the camera eases toward the Stage-7 Pasupati landmark distance `(150,230,-150)` and stays wide until every vehicle at that same metre is destroyed, then eases back. Weapon pickups may aim at all times but may fire only while their own vehicle centre is inside the current camera frustum, within configured range and unobstructed by LOS. Debug/smoke pins the Stage-7 asset provenance, screen direction, frustum gate, camera latch/restore and non-straight route.

**Stage 11 Chapter-1 800 m road/vehicle pass (2026-08-30, user request; supersedes the circular-cap/forest-wave rule):** the authored forest route is exactly 800 metres (`CAMP_M` remains 7 world units/metre). Runtime Three.js draws shoulder and asphalt as continuous indexed ribbons with MITERED vertices at every turn; circular road caps/joint discs are forbidden because they create the bulging, uneven bend. The start is a butt edge and the visible asphalt continues 150 world units straight behind the landing point, while a full-width rail and `stage11ForestWalk` stop the player after 20 units. The concrete/steel bridge follows the route tangent at metre 400. Repeated forest density is 1,600 trees in the SAME four shadowless `InstancedMesh` nodes, plus one shrub batch; never trade it for per-tree meshes. Six stationary hostile double-cabin pickups replace the old forest patrol waves: metre 250 = one MG, metre 500 = one MG plus one homing missile, metre 750 = two MG plus one homing missile. Each pickup carries one real mounted Stage-11 robot. MG tuning lives in `campaign.stage11.forestVehicles.machineGun` and is exactly 20 rounds per burst, 10 damage, then a 2 s burst gap; its 96 tracers are pooled. Missile pickups reuse `buildCombatGunshipMissileMesh`, steer continuously toward the player, are shootable and come from a six-slot prebuilt pool. Vehicle centres must remain on the asphalt. Former rock/log roadblocks are sixteen statically merged pickup cars, never rocks or fallen logs. Smoke `25d` pins route length/join/start boundary, car roadblocks, 1/2/3 checkpoint composition, a real 20-round burst, homing steering and the Chapter 1->2 handoff.
**Stage 11 is one three-chapter mission (2026-08-29, updated 2026-08-30):** `stage11Scene` starts in Chapter 1 under `campaign-11-forest`: Major Gibran descends continuously from configured height, lands at `S11_FOREST_LANDING`, clears the six configured weapon pickups along the 800 m road, then reaches `S11_FOREST_GATE`. Reaching the gate calls `enterStage11Sub(surfaceScene)` directly, and the existing city chapter then hands off to the existing computer/Warden root chapter the same way. Never call `setScene`, a stage transition, or a shop between these chapters: checkpoint 11, loadout, money, stage statistics and facade identity remain continuous. Dialogue `seen` belongs to the entire facade; a cutscene skip may clear the live queue but must not reset that history. The facade/root default and `sceneManager` mapping are `campaign-11-forest`; only one of the forest/surface/root worlds may be active, while inactive roots also skip their matrix walk.
