...
import {
    player, keys, mouse, bullets, zombies, isPaused, isGameOver,
...
export let gunMesh = null, pistolMesh = null;
...
export function initWeapons() {
    ...
    // PENTING: simpan referensi muzzle AK SETELAH objeknya dibuat — attachMuzzle
...
export function attachMuzzle(wpn) {
    const grp = wpn === 'rifle' ? gunMesh : pistolMesh;
    const px = 0, py = wpn === 'rifle' ? 0.1 : 0.42, pz = wpn === 'rifle' ? -7.4 : -2.7;
    grp.add(muzzleFlash);
    muzzleFlash.position.set(px, py, pz);
    grp.add(muzzleSprite);
    muzzleSprite.position.set(px, py, pz - 0.1);
    muzzleSprite.scale.setScalar(wpn === 'rifle' ? 1 : 0.75);
    muzzlePoint = wpn === 'rifle' ? rifleMuzzle : pistolMuzzle;
}
...
// 1 = rifle, 2 = pistol, Q = tukar cepat (dipanggil handler keyboard)
...
export function startReload() {
    ...
    const dur = CFG.weapons[currentWeapon].reloadMs;
...
export function updateWeaponTimers(dt) {
    if (switchAnim >= 0) {
        ...
        if (prev < SWITCH_TIME / 2 && switchAnim >= SWITCH_TIME / 2 && switchTarget) {
            ...
            switchTarget = null;
            gunMesh.visible = currentWeapon === 'rifle';
            pistolMesh.visible = currentWeapon === 'pistol';
...
export function updateWeaponState(dt) {
    ...
    const wGrp = currentWeapon === 'rifle' ? gunMesh : pistolMesh;
    const wBaseZ = currentWeapon === 'rifle' ? -6 : -5;
    const wKick = currentWeapon === 'rifle' ? 2.5 : 1.6;
    // ADS sedikit lebih dekat; melee mendorong senjata ke depan (ayunan)
    wGrp.position.z = wBaseZ + aimT * 0.8 + gunRecoil * wKick - 1.9 * meleeS;
...
export function updateShooting() {
    ...
    const fireDelay = CFG.weapons[currentWeapon].fireDelayMs;
    if (mouse.isDown && !player.isReloading && switchAnim < 0 && meleeT <= 0
        && Date.now() - player.lastShot > fireDelay && wpn.ammo > 0) {
        ...
        _kickEuler.x += CFG.weapons[currentWeapon].cameraKick * acc * (0.8 + Math.random() * 0.4);
        ...
        // Panjang tracer diskalakan dgn kecepatan agar peluru cepat terbaca sbg
        // garis menerus (streak ~ jarak tempuh per frame), bukan titik berjarak.
        ...
        playSFX(currentWeapon === 'rifle' ? sfxShoot : sfxPistol);
...
export function updateWeaponVisuals(dt) {
    ...
    const isRifle = currentWeapon === 'rifle';
    const grp = isRifle ? gunMesh : pistolMesh;
    ...
    const hipX = isRifle ? 3 : 2.6, hipY = isRifle ? -2.5 : -2.3;
    const adsY = isRifle ? -0.78 : -0.7;   // garis pisir masing2 = pusat layar
    grp.position.x = hipX * (1 - aimT) + Math.sin(gunBobT) * bobA;
    grp.position.y = hipY + (adsY - hipY) * aimT + Math.abs(Math.cos(gunBobT)) * bobA * 1.4 + swOff;

    // Rig reload per senjata -> gunRotX/gunRotZ + gerak tangan kiri/mag/bolt/slide.
    // Waktu KF diskalakan ke reloadMs CFG (default 3000/2200 ms = 1:1 dgn basis).
    if (player.isReloading && isRifle) {
        const rt = (Date.now() - reloadStartTime) * 3.0 / CFG.weapons.rifle.reloadMs;   // waktu nyata = sinkron setTimeout
    ...
    } else if (player.isReloading && !isRifle) {
        const rt = (Date.now() - reloadStartTime) * 2.2 / CFG.weapons.pistol.reloadMs;
        ...
        // istirahat: kedua rig kembali menopang; slide pistol kick kecil saat menembak
...
export function resetWeapons() {
    ...
    gunMesh.visible = true; pistolMesh.visible = false;
    attachMuzzle('rifle');
    gunMesh.rotation.set(0, 0, 0);
    pistolMesh.rotation.set(0, 0, 0);
    gunRotX = 0; gunRotZ = 0;
    gunHeat = 0;
}
