...
export function buildDetailedWallCell(add, {
    ...
    for (const face of faces) {
        ...
        if (face.axis === 'x') {
            add(0.48, wallH - 5.4, along, x + facePos, wallH / 2 + 0.2, z, panel);
            add(0.72, 2.8, cell - 0.7, x + facePos + face.sign * 0.08, 2.1, z, ink);
            add(0.68, 1.2, cell - 1, x + facePos + face.sign * 0.1, wallH - 2.1, z, steel);
            for (const y of [7.1, 14.4])
                add(0.72, 0.65, along - 0.5, x + facePos + face.sign * 0.11, y, z, steel);
            for (const dz of [-cell * 0.29, cell * 0.29])
                add(0.72, wallH - 6.6, 0.72, x + facePos + face.sign * 0.12,
                    wallH / 2 + 0.3, z + dz, steel);
            if (accent && hash % accentEvery === 0)
                add(0.76, 3.1, cell * 0.22, x + facePos + face.sign * 0.14,
                    5.3, z + cell * 0.17, accent);
        } else {
            add(along, wallH - 5.4, 0.48, x, wallH / 2 + 0.2, z + facePos, panel);
            add(cell - 0.7, 2.8, 0.72, x, 2.1, z + facePos + face.sign * 0.08, ink);
            add(cell - 1, 1.2, 0.68, x, wallH - 2.1, z + facePos + face.sign * 0.1, steel);
            for (const y of [7.1, 14.4])
                add(along - 0.5, 0.65, 0.72, x, y, z + facePos + face.sign * 0.11, steel);
            for (const dx of [-cell * 0.29, cell * 0.29])
                add(0.72, wallH - 6.6, 0.72, x + dx, wallH / 2 + 0.3,
                    z + facePos + face.sign * 0.12, steel);
            if (accent && hash % accentEvery === 0)
                add(cell * 0.22, 3.1, 0.76, x + cell * 0.17, 5.3,
